  ...
  const loadEmailDrafts = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log('Lead IDs to query from serp_leads_v2:', leadIds);

        if (leadIds.length > 0) {
          console.log('Querying serp_leads_v2 with leadIds:', leadIds);
          console.log('Using authenticated Supabase client');

          // First, verify the lead_ids exist in user_leads for this user
          const { data: userLeadsCheck, error: userLeadsError } = await supabase
            .from('user_leads')
            .select('lead_id')
            .eq('user_id', userAccountId)
            .in('lead_id', leadIds);

          console.log('user_leads verification:', {
            userAccountId,
            requestedLeadIds: leadIds,
            foundInUserLeads: userLeadsCheck?.map(ul => ul.lead_id) || [],
          ...
          if (leadsError) {
            ...
            console.error('Full leadsError object:', JSON.stringify(leadsError, null, 2));
          } else {
            console.log('Successfully fetched leads data:', leadsData);
            const leadsMap = new Map((leadsData || []).map(lead => [lead.id, lead]));
            console.log('Leads map created with keys:', Array.from(leadsMap.keys()));

            transformedData = data.map(draft => {
              const leadInfo = leadsMap.get(draft.lead_id);
              console.log(`Mapping draft ${draft.id} with lead_id ${draft.lead_id}:`, leadInfo);
              return {
                ...draft,
