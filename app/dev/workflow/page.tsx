...
import { WorkflowData, WorkflowSection, WorkflowTask, UserProgress } from '@/types/workflow';
...
import { CheckCircle2, Clock, AlertCircle, ExternalLink, Terminal, BookOpen } from 'lucide-react';
...
export default function WorkflowChecklist() {
  ...
  const calculateOverallProgress = () => {
    ...
    const totalTasks = workflowData.sections.reduce((acc, section) => acc + section.tasks.length, 0);
  ...
  const isTaskBlocked = (task: WorkflowTask) => {
    ...
    return task.dependencies.some(depId => !userProgress.completedTasks.includes(depId));
  };
  ...
  if (!workflowData) {
    return (
      <div className="container mx-auto p-6">
        <div className="text-center text-red-500">Failed to load workflow data</div>
      </div>
    );
  }

  const currentSectionData = workflowData.sections.find(s => s.id === currentSection);

  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">ShipsMind Development Workflow</h1>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-500">
              {userProgress?.completedTasks.length || 0} / {workflowData.sections.reduce((acc, s) => acc + s.tasks.length, 0)} tasks
            </span>
          </div>
          ...
          {currentSectionData && (
            <Card>
              <CardHeader>
                ...
                <Progress value={calculateProgress(currentSectionData)} className="h-2" />
              </CardHeader>
              <CardContent>
                            ...
                            {task.documentation && (
                              <div className="mb-3">
                                <Button variant="outline" size="sm" asChild>
                                  <a href={task.documentation} target="_blank" rel="noopener noreferrer">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {task.dependencies.map((depId) => {
                                    const depTask = workflowData.sections
                                      .flatMap(s => s.tasks)
                                      .find(t => t.id === depId);
                                    const isDepCompleted = userProgress?.completedTasks.includes(depId);
                                    return depTask ? (
                                      <Badge
                                        key={depId}
                                        variant={isDepCompleted ? "default" : "destructive"}
                                        className="text-xs"
                                      >
                                        {depTask.title}
                                      </Badge>
